...
$brand_total     = $db->prepare("SELECT count(id) as toplam FROM Product_Views WHERE brand_url = :url");
$brand_total->execute([':url' => $url]);
$brands_total    = $brand_total->fetch(PDO::FETCH_ASSOC);
$brands_total    = $brands_total['toplam'];
$brand_pages     = ceil($brands_total/$MaxLimit);
/* PAGE LİMİT AYARLARI BİTİŞ */

$product         = $db->prepare("SELECT * FROM Product_Views WHERE brand_url = :url ORDER BY id DESC LIMIT $page_limit, $MaxLimit");
...
<div class="container">
    ...
    <table>
        ...
        <?php
        if(empty($products)){
            echo "<tr><td colspan='8' class='center'>Bu markaya kayıtlı ürün bulunamamıştır.</td></tr>";
        } else {
    ...
    for($i=1; $i<=$brand_pages;  $i++){
        ...
        echo "
        <a href='brands-products/{$url}/{$i}' class='pages {$class}'>{$i}</a>
        ";
    }
    ?>
</div>
